...
parse.imputeinfofile = function(imputeinfofile, is.male, chrom=NA) {
  ...
  #chr_names=unique(impute.info$chrom)      # editied by kimin 20191209 (chrom is passed down as character)
  # Subset for a particular chromosome
  if (!is.na(chrom)) {
    impute.info = impute.info[impute.info$chrom==chrom,]  # editied by kimin 20191209 (chrom is passed down as character)
  }
  return(impute.info)
}
